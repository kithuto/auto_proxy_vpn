...
The GitHub Actions workflow (`.github/workflows/tests.yml`) automates testing on every push and pull request targeting `main`.
...
└─ Unit Tests ──────────────────────── runs on every push & PR
    ...
    Uploads: JUnit XML results + coverage report (XML)

Manual dispatch (workflow_dispatch with "Run integration tests" = true)
│
├─ Unit Tests (same as above)
└─ Integration Tests ──────────────── manual trigger only
    ├─ DigitalOcean (requires secrets)
    ├─ Google Cloud  (requires secrets)
    └─ Azure         (requires secrets)
```

To trigger integration tests, go to **Actions → Tests → Run workflow** and check the "Run integration tests" checkbox.
