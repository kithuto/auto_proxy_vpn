...
class TestGetOrCreateProject:
    ...
    def test_existing_default_project(self):
        responses.add(
            ...
            f"{DO_API}/projects",
            json={
                "projects": [{"id": "proj-id", "name": "MyProj", "is_default": True}]
            },
            status=200,
        )
    ...
    def test_http_error_status_raises(self, monkeypatch):
        ...
        monkeypatch.setattr(
            utils, "get", lambda *args, **kwargs: SimpleNamespace(status_code=500)
        )
...
class TestGetOrCreateSSHKeys:
    ...
    def test_existing_key_by_name(self):
        responses.add(
            ...
            json={
                "ssh_keys": [{"id": 42, "name": "mykey", "public_key": "ssh-rsa AAA"}]
            },
            status=200,
        )
        from auto_proxy_vpn.providers.digitalocean.digitalocean_utils import (
    ...
    def test_creates_key_from_dict(self):
        ...
        keys = get_or_create_ssh_keys(
            [{"name": "newkey", "public_key": "ssh-rsa BBB"}], HEADERS
        )
        assert keys == [99]
    ...
    def test_existing_key_from_dict_uses_existing_id(self):
        responses.add(
            ...
            json={
                "ssh_keys": [
                    {"id": 77, "name": "existing", "public_key": "ssh-rsa AAA"}
                ]
            },
            status=200,
        )
        ...
        keys = get_or_create_ssh_keys(
            [{"name": "existing", "public_key": "ssh-rsa AAA"}], HEADERS
        )
        assert keys == [77]
...
class TestStartProxy:
    def test_non_202_non_422_returns_error(self, monkeypatch):
        ...
        monkeypatch.setattr(
            utils, "post", lambda *args, **kwargs: SimpleNamespace(status_code=500)
        )
    ...
    def test_422_without_retry_raises_country_not_available(self, monkeypatch):
        ...
        monkeypatch.setattr(
            utils, "post", lambda *args, **kwargs: SimpleNamespace(status_code=422)
        )
                ...
                SimpleNamespace(
                    ...
                    json=lambda: {
                        "droplet": {
                            ...
                            "status": "active",
                            "networks": {
                                "v4": [{"type": "public", "ip_address": "8.8.8.8"}]
                            },
                        }
                    },
                ),
            ...
            lambda *args, **kwargs: SimpleNamespace(
                ...
                json=lambda: {
                    "droplet": {
                        ...
                        "status": "new",
                        "networks": {
                            "v4": [{"type": "public", "ip_address": "7.7.7.7"}]
                        },
                    }
                },
            ),
