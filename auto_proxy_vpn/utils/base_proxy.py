...
class BaseProxyManager(ABC, Generic[T]):
    ...
    @abstractmethod
    def from_config(
        cls, config=None, runtime_config: ManagerRuntimeConfig | None = None
    ) -> "BaseProxyManager": ...
    ...
    def get_proxies(
        ...
        if isinstance(ports, list) and len(ports) != number:
            raise ValueError(
                "The length of the ports list must be equal to the number of proxies to create."
            )
        if isinstance(sizes, list) and len(sizes) != number:
            raise ValueError(
                "The length of the sizes list must be equal to the number of proxies to create."
            )
        if isinstance(regions, list) and len(regions) != number:
            raise ValueError(
                "The length of the regions list must be equal to the number of proxies to create."
            )
        if isinstance(auths, list) and len(auths) != number:
            raise ValueError(
                "The length of the auths list must be equal to the number of proxies to create."
            )
        if isinstance(proxy_names, list) and len(proxy_names) != number:
            raise ValueError(
                "The length of the proxy_names list must be equal to the number of proxies to create."
            )
        ...
        if isinstance(sizes, list):
            for size in sizes:
                if size not in ["small", "medium", "large"]:
                    raise ValueError(
                        "Invalid size. Valid sizes are: 'small', 'medium', 'large'."
                    )
        ...
        if isinstance(regions, list):
            for region in regions:
                if region not in self.get_regions_by_size("small"):
                    raise ValueError(
                        "Invalid region! Check the valid regions calling get_regions_by_size()"
                    )
        ...
        for i in range(number):
            ...
            try:
                proxy = self.get_proxy(
                    ...
                    if isinstance(proxy_names, list)
                    else proxy_names,
                    on_exit=on_exit,
                )
            except Exception:
    ...
    def get_regions_by_size(
        self, size: Literal["small", "medium", "large"]
