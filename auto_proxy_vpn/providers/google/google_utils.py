...
def wait_for_extended_operation(operation, timeout: int = 200) -> Any:
    ...
    if operation.warnings:
        print("Warnings during operation:\n", file=stderr, flush=True)
...
def get_avaliable_regions_by_size(
    compute_v1,
    machine_types_client,
    project: str,
    instance_proxy_sizes: dict[Literal["small", "medium", "large"], str],
) -> tuple[
    ...
    region_by_size: dict[
        Literal["small", "medium", "large"], list[str] | list[tuple[str, list[str]]]
    ] = {size: [] for size in instance_proxy_sizes.keys()}
    ...
    request = compute_v1.AggregatedListMachineTypesRequest(
        project=project, filter=f"name={instance_proxy_sizes['small']}"
    )
    ...
    for zone, found_instances in zones:
        if found_instances.machine_types:
            ...
            if zone not in regions_dict[region]:
...
def start_proxy(
    ...
    if firewall:
        ...
        _ = proxy_manager._firewall_client.insert(
            project=proxy_manager.project, firewall_resource=rule
        )
    ...
    config = {
        ...
        "metadata": {
            "items": [
                ...
                },
                {
                    "key": "ssh-keys",
                    "value": "\n".join(
                        [f"ubuntu:{ssh_key}" for ssh_key in proxy_manager.ssh_keys]
                    ),
                },
            ]
        },
        ...
        "network_interfaces": [
            {
                "access_configs": [{"network_tier": "STANDARD"}],
        ...
        "tags": {"items": [proxy_name, "proxy"]},
    }

    request = proxy_manager._compute_v1.InsertInstanceRequest(
        project=proxy_manager.project, zone=zone, instance_resource=config
    )
    ...
    if not is_async:
        try:
            ...
            if proxy_manager.logger:
                proxy_manager.logger.warning(
                    f"Zone {zone} is currently unavailable. Trying to start the proxy in another zone of the same region."
                )
    ...
    instance_request = proxy_manager._compute_v1.GetInstanceRequest(
        instance=proxy_name, project=proxy_manager.project, zone=zone
    )
    proxy_instance_ip = (
        proxy_manager._instances_client.get(instance_request)
